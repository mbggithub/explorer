...
function find_tx(txid, cb) {
  Tx.findOne({txid: txid,vin:{$elemMatch:{$ne:null}}}, function(err, tx) {
    if(tx) {
      return cb(tx);
    } else {
      return cb(null);
...
module.exports = {
  ...
  get_last_txs: function(count, min, cb) {
    Tx.find({vin:{$elemMatch:{$ne:null}},'total':{$gt:0}}).sort({_id: 'desc'}).limit(count).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
  ...
  get_last_txs_reward: function(count, cb) {
    Tx.find({vin:{$size:0},'total':{$gt:0}}).sort({_id: 'desc'}).limit(count).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
